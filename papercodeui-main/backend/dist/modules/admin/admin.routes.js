...
adminRouter.patch('/lessons-reorder', async (req, res, next) => {
    try {
        ...
        await Promise.all(items.map((item) => prisma.lesson.update({ where: { id: item.id }, data: { sortOrder: item.sortOrder } })));
        res.json({ ok: true });
    }
    catch (err) {
        next(err);
...
adminRouter.patch('/chapters-reorder', async (req, res, next) => {
    try {
        const { items } = req.body;
        await Promise.all(items.map((item) => prisma.chapter.update({ where: { id: item.id }, data: { sortOrder: item.sortOrder } })));
        res.json({ ok: true });
    }
    catch (err) {
        next(err);
