...
export default function AdminLessonsPage() {
  ...
  function startEdit(lesson: AdminLesson) {
    ...
    setEditLangId(lesson.languageId);
    setEditDifficulty(lesson.difficulty);
    setEditMinutes(lesson.estimatedMinutes);
    setEditXp(lesson.xpReward);
  }
    ...
    <div className="p-8">
      ...
      {showCreate && (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 shadow-sm space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <Input placeholder="Description (optional)" value={newDesc} onChange={(e) => setNewDesc(e.target.value)} className="rounded-xl" />
          </div>
          <div className="grid sm:grid-cols-4 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Input type="number" min="1" value={newMinutes} onChange={(e) => setNewMinutes(parseInt(e.target.value) || 0)} className="rounded-xl w-full" />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-500 whitespace-nowrap">XP:</span>
              <Input type="number" min="0" value={newXp} onChange={(e) => setNewXp(parseInt(e.target.value) || 0)} className="rounded-xl w-full" />
            </div>
          </div>
      ...
      <div className="space-y-3">
        {loading ? [...Array(4)].map((_, i) => <div key={i} className="h-20 bg-white rounded-2xl border border-gray-100 animate-pulse" />) : filtered.length === 0 ? (
          ...
          editingId === lesson.id ? (
            <div key={lesson.id} className="bg-white rounded-2xl border border-blue-200 p-4 shadow-sm space-y-3">
              <div className="flex flex-col gap-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  <Input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="rounded-xl" placeholder="Lesson title" />
                  <Input value={editDesc} onChange={(e) => setEditDesc(e.target.value)} className="rounded-xl" placeholder="Description" />
                </div>
                <div className="grid sm:grid-cols-5 gap-3 items-center">
                  <select value={editLangId} onChange={(e) => setEditLangId(e.target.value)} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="">Select language</option>{languages.map((l) => <option key={l.id} value={l.id}>{l.name}</option>)}</select>
                  <select value={editDifficulty} onChange={(e) => setEditDifficulty(e.target.value)} className="rounded-xl border border-gray-200 px-3 py-2 text-sm"><option value="BEGINNER">Beginner</option><option value="INTERMEDIATE">Intermediate</option><option value="ADVANCED">Advanced</option></select>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-500 whitespace-nowrap">Time:</span>
                    <Input type="number" min="1" value={editMinutes} onChange={(e) => setEditMinutes(parseInt(e.target.value) || 0)} className="rounded-xl" />
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-500 whitespace-nowrap">XP:</span>
                    <Input type="number" min="0" value={editXp} onChange={(e) => setEditXp(parseInt(e.target.value) || 0)} className="rounded-xl" />
                  </div>
                  <div className="flex gap-2 justify-end">
                    <Button onClick={saveEdit} className="rounded-xl bg-emerald-600 hover:bg-emerald-700">Save</Button>
          ...
          <div key={lesson.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm flex items-center gap-4 hover:border-blue-200 transition-colors">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <Button size="sm" variant="outline" onClick={() => startEdit(lesson)} className="text-gray-600 h-8 px-3 rounded-lg text-xs font-medium">Edit</Button>
